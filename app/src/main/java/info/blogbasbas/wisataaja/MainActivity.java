...
import info.blogbasbas.wisataaja.db.facade.ManageHomeTbl;
import info.blogbasbas.wisataaja.db.facade.ManageWisataTbl;
import info.blogbasbas.wisataaja.model.ResponseWisata;
import info.blogbasbas.wisataaja.model.WisataItem;
import info.blogbasbas.wisataaja.ui.HomeFragment;
...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //inisialisasi
        wisataItem = new ArrayList<>();
        manageWisataTbl = Facade.getInstance().getManageWisataTbl();

        //cara  make atomic
                     Atom.with(this)
                     .load(Constant.BASE_URL)
                   // .load("http://192.168.1.21/Pariwisata/wisata/read_wisata.php")
                    .as(ResponseWisata.class)
                    .setCallback(new FutureCallback<ResponseWisata>() {
                        @Override
                        public void onCompleted(Exception e, ResponseWisata resrult) {
                            if (e != null){
                                Timber.e("Error :" +e.getMessage());
                            }
                            if (resrult.isSuccess()== true){
                                //ambil hasil nama
                                Timber.e(" Hasil dari nama wisata"
                                        +resrult.getWisata().get(0).getNamaWisata());
                                //ambil hasil list wisata
                                Timber.e("Hasil Wisata"
                                    +new Gson().toJson(resrult.getWisata()));
                                    //memasukan data ke tabel wisata
